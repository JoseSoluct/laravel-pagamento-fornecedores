...
class Ted extends AbstractComprovante implements ComprovanteContract
{
    public static function make( DetalheContract $d, array $extra = [] ): string
    {
        ...
        $ted = $d->getPagador()->getDocumento() == $d->getFavorecido()->getDocumento() ? 'Mesma' : 'Outra';

        $data[] = ['Cooperativa Origem:', $d->getContaPagador()->getAgencia()];
        $data[] = ['Conta Origem:', $d->getContaPagador()->getConta() . '-' . $d->getContaPagador()->getContaDv()];
        $data[] = ['Favorecido:', $d->getFavorecido()->getNome()];
        $data[] = ['CPF/CNPJ Favorecido:', $d->getFavorecido()->getDocumento()];
        $data[] = [
            'Instituição:',
            ( $b = $d->getContaFavorecido()->getBanco() ) . ' - ' . Util::bancoNome( $b )
        ];
        $data[] = ['Cooperativa/Agência:', $d->getContaFavorecido()->getAgencia()];
        $data[] = [
            'Conta Destino:',
            $d->getContaFavorecido()->getConta() . '-' . $d->getContaFavorecido()->getContaDv()
        ];
        $data[] = ['Data Transferência:', $d->getDataPagamento()];
        $data[] = ['Valor a Transferir (R$):', number_format( $d->getValorPagamento(), 2, ',', '.' )];
        $data[] = ['Motivo Transferência:', $extra['motivo'] ?? ''];
